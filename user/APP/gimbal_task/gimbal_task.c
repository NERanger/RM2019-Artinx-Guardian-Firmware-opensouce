...
#define gimbal_total_pid_clear(gimbal_clear)                                                   \
    {                                                                                          \
		...
		PID_clear(&(gimbal_clear)->gimbal_yaw_motor.gimbal_motor_test_spd_pid);                \
                                                                                               \
...
Gimbal_Control_t debug_gimbal_control;
uint16_t debug_rawecd;
uint16_t debug_tmp;
...
static void gimbal_motor_test_spd_control (Gimbal_Motor_t *gimbal_motor);
static void gimbal_motor_no_force_control (Gimbal_Motor_t *gimbal_motor);
//Added by NERanger 20190410
static void GIMBAL_relative_angle_limit_m3508(Gimbal_Motor_t *gimbal_motor, fp32 add);  
static void gimbal_motor_relative_angle_control_m3508(Gimbal_Motor_t *gimbal_motor);
static int16_t m3508_gimbal_get_ecd_angle(int32_t total_ecd, int16_t center_offset);
...
void GIMBAL_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (!(toe_is_error(YawGimbalMotorTOE) && toe_is_error(PitchGimbalMotorTOE) && toe_is_error(TriggerMotorTOE)))
        {
			...
			else if(switch_is_down(gimbal_control.gimbal_rc_ctrl->rc.s[ModeChannel]))
			{
				CAN_CMD_GIMBAL(0, 0, 0, 0);
			}
			else
...
{
    gimbal_control.gimbal_yaw_motor.offset_ecd = YAW_OFFSET_MANUAL;
//    gimbal_control.gimbal_yaw_motor.max_relative_angle = max_yaw;
//    gimbal_control.gimbal_yaw_motor.min_relative_angle = min_yaw;
//	gimbal_control.gimbal_yaw_motor.max_relative_angle = MAX_YAW_RELATIVE_ANGLE_MANUAL_SET;
//	gimbal_control.gimbal_yaw_motor.min_relative_angle = MIN_YAW_RELATIVE_ANGLE_MANUAL_SET;

    gimbal_control.gimbal_pitch_motor.offset_ecd = pitch_offset;
    gimbal_control.gimbal_pitch_motor.max_relative_angle = MAX_PITCH_RELATIVE_ANGLE_MANUAL_SET;
    gimbal_control.gimbal_pitch_motor.min_relative_angle = MIN_PITCH_RELATIVE_ANGLE_MANUAL_SET;
}
...
static void GIMBAL_Init(Gimbal_Control_t *gimbal_init)
{
	...
	PID_Init(&gimbal_init->gimbal_yaw_motor.gimbal_motor_test_spd_pid, PID_POSITION, motor_speed_pid, M3505_MOTOR_SPEED_PID_MAX_OUT, M3505_MOTOR_SPEED_PID_MAX_IOUT);
	//Added by NERanger 20190410
	...
	gimbal_init->gimbal_yaw_motor.max_relative_angle = gimbal_init->gimbal_yaw_motor.relative_angle_set + MAX_YAW_RELATIVE_ANGLE_MANUAL_ADD;
	gimbal_init->gimbal_yaw_motor.min_relative_angle = gimbal_init->gimbal_yaw_motor.relative_angle_set + MIN_YAW_RELATIVE_ANGLE_MANUAL_ADD;
	...
	gimbal_init->gimbal_pitch_motor.motor_speed_set = gimbal_init->gimbal_pitch_motor.motor_speed;// Added by NERanger 20190422
...
static void GIMBAL_Feedback_Update(Gimbal_Control_t *gimbal_feedback_update)
{
	...
	gimbal_feedback_update->gimbal_pitch_motor.motor_speed = gimbal_feedback_update->gimbal_pitch_motor.gimbal_motor_measure->speed_rpm;
...
//                                                                                        gimbal_feedback_update->gimbal_yaw_motor.offset_ecd);
	...
	gimbal_feedback_update->gimbal_yaw_motor.relative_angle = motor_ecd_to_angle_change(m3508_gimbal_get_ecd_angle(gimbal_feedback_update->gimbal_yaw_motor.gimbal_motor_measure->total_ecd, gimbal_feedback_update->gimbal_yaw_motor.offset_ecd),
																						gimbal_feedback_update->gimbal_yaw_motor.offset_ecd);
	//Modified by NERanger 20190413
	...
	gimbal_feedback_update->gimbal_yaw_motor.motor_speed = CHASSIS_MOTOR_RPM_TO_VECTOR_SEN * (gimbal_feedback_update->gimbal_yaw_motor.gimbal_motor_measure->speed_rpm);
	//Added by NERanger 20190411
}
//计算相对角度
static fp32 motor_ecd_to_angle_change(uint16_t ecd, uint16_t offset_ecd)
...
static void GIMBAL_Mode_Change_Control_Transit(Gimbal_Control_t *gimbal_mode_change)
{
    ...
    else if (gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_ENCONDE_M3508 && gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE_M3508)
    {
...
static void GIMBAL_Set_Contorl(Gimbal_Control_t *gimbal_set_control)
{
	...
	else if (gimbal_set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_NO_FORCE)  //Added by NERanger 20190410
	{
		gimbal_set_control->gimbal_yaw_motor.raw_cmd_current = 0;
	}
	else if (gimbal_set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_SPD)       //Added by NERanger 20190410
	{
		gimbal_set_control->gimbal_yaw_motor.raw_cmd_spd = add_yaw_angle;
	}
	else if (gimbal_set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE_M3508)  //Added by NERanger 20190413
	{
		GIMBAL_relative_angle_limit_m3508(&gimbal_set_control->gimbal_yaw_motor, add_yaw_angle);
	}
	...
	else if (gimbal_set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_NO_FORCE)  //Added by NERanger 20190410
	{
		gimbal_set_control->gimbal_yaw_motor.raw_cmd_current = 0;
	}
	else if (gimbal_set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_SPD)       //Added by NERanger 20190427
	{
		gimbal_set_control->gimbal_pitch_motor.raw_cmd_spd = add_pitch_angle;
	}
}
//陀螺仪 控制量限制
static void GIMBAL_absolute_angle_limit(Gimbal_Motor_t *gimbal_motor, fp32 add)
...
static void GIMBAL_relative_angle_limit_m3508(Gimbal_Motor_t *gimbal_motor, fp32 add)   // Added by NERanger 20190413
{
    if (gimbal_motor == NULL)
    {
        return;
...
static void GIMBAL_Control_loop(Gimbal_Control_t *gimbal_control_loop)
{
	...
	else if (gimbal_control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_SPD)
	{
//		PID_Calc(gimbal_control_loop->gimbal_yaw_motor.gimbal_motor_test_spd_pid, gimbal_control_loop->gimbal_yaw_motor.motor_speed, );
		gimbal_motor_test_spd_control(&gimbal_control_loop->gimbal_yaw_motor);
	}
	else if (gimbal_control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_NO_FORCE)
	{
		gimbal_motor_no_force_control(&gimbal_control_loop->gimbal_yaw_motor);
	}
	else if (gimbal_control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE_M3508)
	{
		gimbal_motor_relative_angle_control_m3508(&gimbal_control_loop->gimbal_yaw_motor);
	}
...
//    gimbal_motor->current_set = PID_Calc(&gimbal_motor->gimbal_motor_gyro_pid, gimbal_motor->motor_gyro, gimbal_motor->motor_gyro_set);
	...
	gimbal_motor->motor_speed_set = GIMBAL_PID_Calc(&gimbal_motor->gimbal_motor_relative_angle_pid, gimbal_motor->relative_angle, gimbal_motor->relative_angle_set, gimbal_motor->motor_speed);
	gimbal_motor->current_set = PID_Calc(&gimbal_motor->gimbal_motor_gyro_pid, gimbal_motor->motor_speed, gimbal_motor->motor_speed_set);
...
static void gimbal_motor_relative_angle_control_m3508(Gimbal_Motor_t *gimbal_motor)
{
	...
	gimbal_motor->motor_speed_set = GIMBAL_PID_Calc(&gimbal_motor->gimbal_motor_relative_angle_pid, gimbal_motor->relative_angle, gimbal_motor->relative_angle_set, gimbal_motor->motor_speed);
	gimbal_motor->current_set = PID_Calc(&gimbal_motor->gimbal_motor_test_spd_pid, gimbal_motor->motor_speed, gimbal_motor->motor_speed_set);
...
static void gimbal_motor_test_spd_control (Gimbal_Motor_t *gimbal_motor)
{
    ...
    gimbal_motor->current_set = PID_Calc(&gimbal_motor->gimbal_motor_gyro_pid, gimbal_motor->motor_speed, gimbal_motor->raw_cmd_spd);
    //控制值赋值
    gimbal_motor->given_current = (int16_t)(gimbal_motor->current_set);
	//PID_Calc(&gimbal_motor->gimbal_motor_test_spd_pid, gimbal_motor->gimbal_motor_measure->speed_rpm, gimbal_motor->raw_cmd_spd);
	//gimbal_motor->given_current = (int16_t)(gimbal_motor->gimbal_motor_test_spd_pid.out);
}
...
static int16_t m3508_gimbal_get_ecd_angle(int32_t total_ecd, int16_t center_offset)
{
  ...
  int16_t raw_ecd = 0;
	...
	//total_ecd += center_offset;
	
  int32_t abs_total_ecd = int_abs(total_ecd);
	
	if(total_ecd > 0)
	{
		if(total_ecd <= ONE_ROUND_ECD_WITH_REDUCTION)
		{
			raw_ecd = total_ecd / REDUCTION_RATIO_FAC;
		}
		else if(total_ecd > ONE_ROUND_ECD_WITH_REDUCTION)
		{	
			raw_ecd = (total_ecd - total_ecd/ONE_ROUND_ECD_WITH_REDUCTION * ONE_ROUND_ECD_WITH_REDUCTION) / REDUCTION_RATIO_FAC;
		}
	
	}
	else
	{
		if(abs_total_ecd <= ONE_ROUND_ECD_WITH_REDUCTION)
		{
			raw_ecd = ONE_ROUND_ECD - abs_total_ecd / REDUCTION_RATIO_FAC;
		}
		else if(abs_total_ecd > ONE_ROUND_ECD_WITH_REDUCTION)
		{
			raw_ecd = ONE_ROUND_ECD - (abs_total_ecd - total_ecd/ONE_ROUND_ECD_WITH_REDUCTION * ONE_ROUND_ECD_WITH_REDUCTION) / REDUCTION_RATIO_FAC;
		}
	}
	
	debug_rawecd = raw_ecd;
